...
def getCtAugmentedCandidate(
    ...
    ct_t = torch.from_numpy(ct_chunk).unsqueeze(0).unsqueeze(0).to(torch.float32)  # add batch size, channel
...
class LunaDataset(Dataset):
    ...
    def __len__(self):
        totalSamples = len(self.candidateInfo_list)
        return int(min(10000, totalSamples)) if self.ratio_int else totalSamples
